...
"""
    ...
    B=4*(n.*nk).^2./(nnk2.*nnk2-nnk2); ab=[A.T [2; B1; B.T]]; s=sqrt(ab(2:N,2));
    [V,X]=eig(diag(ab(1:N,1),0)+diag(s,-1)+diag(s,1));
    [X,I]=sort(diag(X)); x=(X+1)/2; wx=ab(1,2)*V(1,I).T.^2/4;
    N=N-1; N1=N+1; N2=N+2;  y=cos((2*(N:-1:0).T+1)*pi/(2*N+2));
    L=zeros(N1,N2);  y0=2;  iter=0;
    while max(abs(y-y0))>eps    
        L(:,1)=1;    L(:,2)=y;   
        for k in range(2,N1): #k=2:N1
